		...
		if [ $ONLYCUTS -eq 0 ]; then 
			...
			if [ $ONLYCORRECTION -eq 0 ]; then
				...
				if [ $MCFILE -eq 1 ]; then 
					...
					root -x -q -l -b TaskV1/LineShapeDalitzV1.C\+\+\(\"$Pi0dataRAWFILE\"\,\"$Pi0MCRAWFILE\"\,\"$cutSelection\"\,\"Pi0\"\,\"$Suffix\"\,\"$energy\"\)
				fi	
				...
				if [ $Noeta -eq 0  ] && [ "$energy" != "PbPb_2.76TeV" ] && [ "$energy" != "pPb_5.023TeV" ]; then
					if [ $NoPi0Eta -eq 0 ]; then
						...
						if [ $MCFILE -eq 1 ]; then 
							...
							if [ $MERGINGMC -eq 1 ]; then
							 ...
							 root -x -q -l -b TaskV1/LineShapeDalitzV1.C\+\+\(\"$Pi0EtadataRAWFILE\"\,\"$Pi0EtaMCRAWFILE\"\,\"$cutSelection\"\,\"Pi0EtaBinning\"\,\"$Suffix\"\,\"$energy\"\)
						fi
					fi
					...
					if [ $MCFILE -eq 1 ]; then 
						...
						if [ $MERGINGMC -eq 1 ]; then
		 				...
		 				root -x -q -l -b TaskV1/LineShapeDalitzV1.C\+\+\(\"$EtadataRAWFILE\"\,\"$EtaMCRAWFILE\"\,\"$cutSelection\"\,\"Eta\"\,\"$Suffix\"\,\"$energy\"\)
					fi
