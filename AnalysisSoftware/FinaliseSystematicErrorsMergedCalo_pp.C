            ...
            if (nameCutVariationSC[i].CompareTo("ClusterTrackMatchingCalo")==0 ){
                ...
                for (Int_t k = 0;k < nPtBins;k++){
                    ...
                    if (additionalNameOutput.Contains("INT1") ||
                        ...
                        if (additionalNameOutput.Contains("NLM1"))
                            error   = 2.2+(-0.01)*ptBins[k]+(0.005)*ptBins[k]*ptBins[k];
                        else if (additionalNameOutput.Contains("NLM2"))
           ...
           // manual smoothing for cluster shape errors - variation 1
            if (nameCutVariationSC[i].CompareTo("ClusterM02")==0 ){//&& meson.Contains("Pi0")
                ...
                if (additionalNameOutput.Contains("NLM1")){
                    for (Int_t k = 0;k < nPtBins;k++){
                        if ( ptBins[k] > 12 ){
                            Double_t error              = 2.3+(-0.02)*ptBins[k]+(0.007)*ptBins[k]*ptBins[k];
                            errorsMean[i][k]            = error;
            ...
            if (nameCutVariationSC[i].CompareTo("ClusterNonLinearity")==0 ){//&& meson.Contains("Pi0")
                ...
                for (Int_t k = 0;k < nPtBins;k++){
                    ...
                    if ( (  additionalNameOutput.Contains("INT1")     || 
                       ...
                       error   = 3.4+(0.01)*ptBins[k]+(0.004)*ptBins[k]*ptBins[k]; 
                    }  
                    errorsMean[i][k]            = error;
                    errorsMeanErr[i][k]         = error*0.01;
                    errorsMeanCorr[i][k]        = error;
            ...
            if (nameCutVariationSC[i].CompareTo("MesonMass")==0 ){
                ...
                if (additionalNameOutput.Contains("NLM2") ){
                    ...
                    for (Int_t k = 0;k < nPtBins;k++){
                        if ( ptBins[k] > 10 ){
                            Double_t error              = 0;
                            error                       = 2.+(0.001)*ptBins[k]+(0.004)*ptBins[k]*ptBins[k];
                            errorsMean[i][k]            = error;
                            errorsMeanErr[i][k]         = error*0.01;
                            errorsMeanCorr[i][k]        = error;
                            errorsMeanErrCorr[i][k]     = error*0.01;
            ...
            if (nameCutVariationSC[i].CompareTo("MesonAlpha")==0 ){
                ...
                if (additionalNameOutput.Contains("NLM2") ){
                    ...
                    for (Int_t k = 0;k < nPtBins;k++){
                        if ( ptBins[k] > 11 ){
                            Double_t error              = 0;
                            error                       = 1.+(0.001)*ptBins[k]+(0.0015)*ptBins[k]*ptBins[k];
                            errorsMean[i][k]            = error;
                            errorsMeanErr[i][k]         = error*0.01;
                            errorsMeanCorr[i][k]        = error;
                            errorsMeanErrCorr[i][k]     = error*0.01;
            ...
            if (nameCutVariationSC[i].CompareTo("ClusterMaterialTRD")==0 ){
                ...
                Double_t error                  = 7.07; //(5% for TRD mat, 5% for TOF mat added in quadrature)
                for (Int_t k = 0;k < nPtBins;k++){
                    errorsMean[i][k]            = error;
                    errorsMeanErr[i][k]         = error*0.01;
                    errorsMeanCorr[i][k]        = error;
            ...
            if (nameCutVariationSC[i].CompareTo("Trigger") == 0){
                ...
                for (Int_t k = 0;k < nPtBins;k++){
                    ...
                    if (additionalNameOutput.Contains("INT1") ||
                        ...
                        error   = TMath::Sqrt(8.59*8.59+2*2);
                    } else if (additionalNameOutput.Contains("EMC7")){
                        error   = TMath::Sqrt(3.936*3.936*4.208+2*2);
                    } else if (additionalNameOutput.Contains("EG2")){
                        error   = TMath::Sqrt(5.08*5.08+2*2);
                    } else if (additionalNameOutput.Contains("EG1")){
                        error   = TMath::Sqrt(13.0*13.0+2*2);
                    }    
...
//             if (additionalNameOutput.Contains("EMC1"))  minPt = 2.6;
...
//         }
//         // trigger errors
//         if (numberCutStudies>10 && !(additionalNameOutput.CompareTo("INT1") == 0 || additionalNameOutput.CompareTo("INT7") ==0 ) && meson.CompareTo("EtaToPi0") != 0){
...
//     canvasSummedErrMean->SaveAs(Form("SystematicErrorsCalculatedMergedCalo/SysErrorSummedVisu_%s_%s%s_%s.%s",meson.Data(), energy.Data(),additionalNameOutput.Data(),dateForOutput.Data(),suffix.Data()));
//     
