#include "QA.h"
/*
Takes the input parameters and runs the appropriate cuts based on the GammaConv output file. Things to keep in mind while using this function is that the directory and data set paths and name are correct and fit to you work area, select the mode, cutNr, or doing extending QA. Only difference bewtween the EventQA.C and this file is that EventQA.C handles the entire merged output, while this one works on a run by run bases. This uses histograms which resulted from the GammaConv QA. 
 */
void EventQA_Runwise_ddixit(
                Int_t nSetsIn,
                Int_t nDataIn,
		TString fEnergyFlag,
		TString filePath,
		...
		Int_t mode = 2,                                 //defined below (around line 77)
			    ...
			    )
{
  ...
  const Int_t nSets = nSetsIn;//=10
  const Int_t nData = nDataIn;//=10
  ...
  */
  ...
  TString fileRuns = Form("/home/dhruv/afterburner/AnalysisSoftware/runNumbers%s.txt", (vecDataSet.at(0)).Data());
  if(!readin(fileRuns, vecRuns, kFALSE)) 
    {
      cout << "ERROR, no Run Numbers could be found! Returning..." << endl; 
      return;
    }
  ...
  if(nameMainDir.IsNull() || !nameMainDir.BeginsWith("Gamma"))
    {
      cout << "ERROR, Unable to obtain valid name of MainDir:|" << nameMainDir.Data() << "|, running in mode: " << fMode << endl; 
      return;
    }
  ...
  if(!listInput) 
    {
      cout << "ERROR: Could not find main dir: " << nameMainDir.Data() << " in file! Returning..." << endl; 
      return;
    }
  ...
  for(Int_t i = 0; i < (Int_t) cuts.size(); i++) 
    {
      cout << Form("(%i) -- %s", i, cuts[i].Data()) << endl;
    }
  ...
  if(isCalo)
	...
	}//end fClusterCutSelection.BeginsWith('1')
	...
	}//end fClusterCutSelection.BeginsWith('2')

      else 
	{
	  cout << "No correct calorimeter type found: " << calo.Data() << ", returning..." << endl;
	  return;
	}
    }//end isCalo
	...
	{
		vecRuns.clear();
		fileRuns = Form("/home/dhruv/afterburner/AnalysisSoftware/runNumbers%s.txt", vecDataSet.at(i).Data());
		
		//cout << __FILE__ << __LINE__ << endl;
		
		if(useDataRunListForMC && i>=nData) 
		  {
		    fileRuns = Form("/home/dhruv/afterburner/AnalysisSoftware/runNumbers%s-%s.txt", vecDataSet.at(i).Data(),vecDataSet.at(0).Data());
		    cout << "Switch useDataRunListForMC is true, reading runs from: " << fileRuns.Data() << endl;
		  }
		if(!readin(fileRuns, vecRuns, kFALSE)) 
		  {
		    cout << "ERROR, no Run Numbers could be found! Returning..." << endl; 
		    printf("/home/dhruv/afterburner/AnalysisSoftware/runNumbers%s.txt", vecDataSet.at(i).Data());
		    return;
		  }
	  ...
	  {
	    rangesRuns[i][0]=0; rangesRuns[i][1]=0; 
	  }
	}
	//cout << __FILE__ << __LINE__ << "\tMade it outside the if statement" << endl;
...
//****************************** Looping over DataSets ************************************************
	...
	fileRuns = Form("/home/dhruv/afterburner/AnalysisSoftware/runNumbers%s.txt", fDataSet.Data());
	if(useDataRunListForMC && i>=nData) 
	  {
	    fileRuns = Form("/home/dhruv/afterburner/AnalysisSoftware/runNumbers%s-%s.txt", vecDataSet.at(i).Data(),vecDataSet.at(0).Data());
	    cout << "Switch useDataRunListForMC is true, reading runs from: " << fileRuns.Data() << endl;
	...
	for(Int_t j=0; j<(Int_t) vecRuns.size(); j++)
	  {
	    ...
	    if( nEvents < 1 )
	      {
		cout << "Warning: number of accepted events in run: " << nEvents << "! Setting nEvents to 1..." << endl; 
		nEvents = 1;
	      }
		...
		hTracksRMSGood[i]->SetBinError(bin, GOODESD->GetRMSError());
	      ...
	      cout << "INFO: Object |GoodESDTracks| could not be found! Skipping Fill..." << endl;
	//---------
