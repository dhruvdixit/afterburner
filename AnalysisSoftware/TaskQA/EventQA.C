				...
				)
{
  ...
  TString nameMainDir[maxSets];//stores the keys from the root file which is used to obtain the cut number
  ...
  //			10 // merged EMCal
  //			11 // merged PHOS
  if(fMode == 0 || fMode == 1 || fMode == 2 || fMode == 3 || fMode == 9) isGamma = kTRUE;//all the modes with PCM
  if(fMode == 2 || fMode == 3 || fMode == 4 || fMode == 5 || fMode == 10 || fMode == 11) isCalo = kTRUE;//all the modes with the calorimeters (EMCAL and/or PHOS)
  
  TString fCutSelection[nSets];
  TString fEventCutSelection[nSets];
  TString fGammaCutSelection[nSets];
  TString fClusterCutSelection[nSets];
  TString fElectronCutSelection[nSets];
  TString fMesonCutSelection[nSets];
