...
void AnalysisQA(TString mode = "allrun", TString fileName = "PhotonQA_501000102209297002322000000_Data.root", TString cutNumber = "501000102209297002322000000", TString cutNumber2 = "501000102209297002322000000", TString fileDirectory = "/home/admin1/leardini/photonconv/AnalysisSoftware/f010", TString outputDirectory = "/home/admin1/leardini/OutputAnalysisQA", TString suffix = "eps", TString runFile = "RunNumbers.txt")
...
{ 
   if(mode.EqualTo("allrun"))
	...
	TDirectoryFile *folderInFile = (TDirectoryFile*) File->Get(Form("GammaConv_%s",cutNumber.Data()));
	cout << Form("GammaConv_%s",cutNumber.Data()) << endl;
	if (!folderInFile) return;
   
	TH3F *ElectrondEdxEtaP = (TH3F*) folderInFile->Get("histoElectrondEdxEtaP")->Clone("ElectrondEdxEtaP");        
	cout << ElectrondEdxEtaP << endl;
	TH3F *ElectronNSigmadEdxEtaP = (TH3F*) folderInFile->Get("histoElectronNSigmadEdxEtaP")->Clone("ElectronNSigmadEdxEtaP");        
	cout << ElectronNSigmadEdxEtaP << endl;
	TH3F *PositrondEdxEtaP = (TH3F*) folderInFile->Get("histoPositrondEdxEtaP")->Clone("PositrondEdxEtaP");        
	cout << PositrondEdxEtaP << endl;
	TH3F *PositronNSigmadEdxEtaP = (TH3F*) folderInFile->Get("histoPositronNSigmadEdxEtaP")->Clone("PositronNSigmadEdxEtaP");        
	cout << PositronNSigmadEdxEtaP << endl;
	TH3F *histonSigdEdxElnSigdEdxPosGammaP = (TH3F*) folderInFile->Get("histonSigdEdxElnSigdEdxPosGammaP")->Clone("nSigdEdxElnSigdEdxPosGammaP");        
	cout << PositronNSigmadEdxEtaP << endl;
	
	//nSigmaDaughter Slices in pT
	TH2D* nSigmadEdxElnSigmadEdxPosPtSlice[20];
	Double_t lowRange;
	Double_t highRange;
	for(int i=0;i<20;i++){
		histonSigdEdxElnSigdEdxPosGammaP->GetZaxis()->SetRangeUser(0,50);
		lowRange=i*0.2; 
		highRange=(1+i)*0.2; 
		cout << lowRange <<" to "<< highRange<< endl;
		histonSigdEdxElnSigdEdxPosGammaP->GetZaxis()->SetRangeUser(lowRange,highRange);
		nSigmadEdxElnSigmadEdxPosPtSlice[i] = (TH2D*)histonSigdEdxElnSigdEdxPosGammaP->Project3D("xyo");
		nSigmadEdxElnSigmadEdxPosPtSlice[i]->SetName(Form("nSigmaDaughterSlice%fto%f",lowRange,highRange));
	}	
	//dE/dx versus Pt
	TH2D *ElectrondEdxPt = (TH2D*)ElectrondEdxEtaP->Project3D("xz");
	cout << "here" << endl;
	TH2D *PositrondEdxPt = (TH2D*)PositrondEdxEtaP->Project3D("xz");
	cout << "here" << endl;
	//dE/dx versus Eta
	TH2D *ElectrondEdxEta = (TH2D*) ElectrondEdxEtaP->Project3D("xy");
	cout << "here" << endl;
	TH2D *PositrondEdxEta = (TH2D*) PositrondEdxEtaP->Project3D("xy");
	cout << "here" << endl;
	//NSigma dE/dx versus Pt 
	TH2D *ElectronNSigmadEdxPt = (TH2D*) ElectronNSigmadEdxEtaP->Project3D("xz");
	TH2D *PositronNSigmadEdxPt = (TH2D*) PositronNSigmadEdxEtaP->Project3D("xz");
	cout << "here" << endl;
	//NSigma dE/dx versus Eta
	TH2D *ElectronNSigmadEdxEta = (TH2D*) ElectronNSigmadEdxEtaP->Project3D("xy");
	TH2D *PositronNSigmadEdxEta = (TH2D*) PositronNSigmadEdxEtaP->Project3D("xy");
	cout << "here" << endl;
	//dE/dx distribution
	TH1D *ElectrondEdx = (TH1D*) ElectrondEdxEtaP->Project3D("x");
	TH1D *PositrondEdx = (TH1D*) PositrondEdxEtaP->Project3D("x");	
	cout << "here" << endl;
	//NSigma dE/dx distribution
	TH1D *ElectronNSigmadEdx = (TH1D*) ElectronNSigmadEdxEtaP->Project3D("x");
	TH1D *PositronNSigmadEdx = (TH1D*) PositronNSigmadEdxEtaP->Project3D("x");
...
//	TH1D *PositronEta = (TH1D*) PositrondEdxEtaP->Project3D("y");	
	...
	TH1D *ElectronEta_NSigma = (TH1D*) ElectronNSigmadEdxEtaP->Project3D("y");
	TH1D *PositronEta_NSigma = (TH1D*) PositronNSigmadEdxEtaP->Project3D("y");
...
//	TH1D *PositronPt = (TH1D*) PositrondEdxEtaP->Project3D("z");	
	...
	//Eta distribution (from NSigma dE/dx)
	TH1D *ElectronPt_NSigma = (TH1D*) ElectronNSigmadEdxEtaP->Project3D("z");
	TH1D *PositronPt_NSigma = (TH1D*) PositronNSigmadEdxEtaP->Project3D("z");
...
// 	GammaCandidates->Write();
	...
	for(int i=0;i<20;i++){
		nSigmadEdxElnSigmadEdxPosPtSlice[i]->Write();
	}
...
// 	GammaCandidateFromData->Close();
     ...
     if(mode.EqualTo("runwise"))
	{
	   ...
	   for(Int_t i=0; i<counter+1; i++)
		...
		folderInFile[i] = (TDirectoryFile*) File[i]->Get(Form("GammaConv_%s",cutNumber.Data()));
	    ...
	    //EventFile[i] = new TFile(Form("%s/%s/PhotonQA.root", fileDirectory.Data(),RunNumbers[i].Data()));
		...
		//get the number of events
			...
			//PreEvFolder[i] = (TDirectory*) EventFile[i]->Get(Form("GammaConvV1_QA_%s",cutNumber2.Data()));  //used for different PhotonQA fileformat
	      	//EvFolder[i] = (TList*) PreEvFolder[i]->Get(Form("GammaConv_V1QA_%s",cutNumber2.Data()));		  //used for different PhotonQA fileformat
