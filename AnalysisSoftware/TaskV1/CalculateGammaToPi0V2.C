...
// 		histoGammaSpecCorrPurity = (TH1D*)fileGamma->Get("GammaUnfoldPileUp");
	...
	cout << "qui!"<< endl;
	if(!histoGammaSpecCorrPurity) cout << "non ci sta" << endl;
	...
	if (doNLOComparison){
		...
		SetHistogramm(histRatioNLOMuHalf,"p_{T} (GeV/c)","#frac{Decay #gamma}{NLO Direct #gamma}",1e2,5e4);
		SetHistogramm(histRatioNLOMuOne,"p_{T} (GeV/c)","#frac{Decay #gamma}{NLO Direct #gamma}",1e2,5e4);
		SetHistogramm(histRatioNLOMuTwo,"p_{T} (GeV/c)","#frac{Decay #gamma}{NLO Direct #gamma}",1e2,5e4);
...
// 		return;
		...
		fileFinalResults << forOutput.Data() << endl;
		fitEtaYieldB->SetLineColor(kRed+2);
...
// 		fitPhiYield->SetParLimits(1,5,9);
	...
	TLegend* leg_ConversionSpeccanvas = GetAndSetLegend(0.45,0.63,6);
...
// 	TCanvas *FitRatioPi0ToChargedCanvas = new TCanvas();
	...
	cocktailDir = (TDirectoryFile*) cocktailFile->Get(fcocktailFunc);
	...
	cocktailPi0 = (TH1D* )cocktailDir->Get("ptPi0");
	...
	TLegend* leg_MesonSpectra = GetAndSetLegend(0.7,0.75,3);
	leg_MesonSpectra->AddEntry(cocktailPi0,"Cocktail #pi^{0}");
	leg_MesonSpectra->AddEntry(fitPi0YieldC,"fit to #pi^{0} data","l");
	if(cocktailEta) leg_MesonSpectra->AddEntry(cocktailEta,"Cocktail #eta","l");
	...
	cocktailPi0Gamma = (TH1D* )cocktailDir->Get("ptgPi0");
...
// 		canvasConversionDoubleRatioSum->Print(Form("%s/%s_DoubleRatio_Sum_%s.%s",outputDir.Data(),textPi0New.Data(),centralityAdd.Data(),suffix.Data()));
		...
		fileCorrectedOutput = new TFile(nameOutputFile,"RECREATE");
			...
			// Double ratio
			histoDoubleRatioFitPi0YieldPurity->Write();
			histoDoubleRatioFitPi0YieldPurityWide->Write();
			histoDoubleRatioFitPi0YieldPurityNarrow->Write();
			histoDoubleRatioConversionHighFitPurity->Write();
			histoDoubleRatioConversionLowFitPurity->Write();
			histoDoubleRatioFitPi0YieldPurityModA->Write();
			histoDoubleRatioFitPi0YieldPurityModB->Write();
			//histoDoubleRatioCombinedPurity->Write();
			histoDoubleRatioConversionTrueEffPurity->Write();
			histoDoubleRatioConversionTrueEffPurityWide->Write();
			histoDoubleRatioConversionTrueEffPurityNarrow->Write();
			histoDoubleRatioConversionTrueEffPurityModA->Write();
			histoDoubleRatioConversionTrueEffPurityModB->Write();
