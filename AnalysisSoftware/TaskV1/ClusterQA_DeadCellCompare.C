...
//							};
	...
	for(Int_t j=0; j<nSets; j++)
	{
		...
		if(nMCSets>0){
			for(Int_t iMC=0; iMC<nMCSets; iMC++){
				runs.clear();
				fileRuns = Form("runNumbers%s-%s.txt",MCSets[iMC].Data(),DataSets[j].Data());
				if(!readin(fileRuns, runs, kFALSE)) {cout << Form("INFO, no Run Numbers could be found for %s-%s!",MCSets[iMC].Data(),DataSets[j].Data()) << endl;}

				fLogRunwiseDeadCells.open(Form("%s/%s/ClusterQA/%s/Runwise/ColdCellsRunwise-%s-%s.log",MCcut[iMC].Data(),fEnergyFlag.Data(),suffix.Data(),MCSets[iMC].Data(),DataSets[j].Data()), ios::in);
				if(fLogRunwiseDeadCells.good())
				{
					...
					cout << "\tCould not open DeadCellsRunwise file for: " << MCSets[iMC].Data() << "-" << DataSets[j].Data() << endl;
				}
